...
def recolorRC(src, dst):
    ...
    """
    b, g, r = cv2.split(src)
    cv2.addWeighted(b, 0.5, g, 0.5, 0, b)  # 两张图片线性混合
    cv2.merge((b, b, r), dst)  # 再将处理后的信道合并
...
def recolorRGV(src, dst):
    ...
    """
    b, g, r = cv2.split(src)  # b信道取三个信道中的最小值
    cv2.min(b, g, b)
    cv2.min(b, r, b)
    cv2.merge((b, g, r), dst)
...
def recolorCMV(src, dst):
    ...
    Pseudocode:
    dst.b = max(src.b, src.g, src.r)# b信道为三个信道的最大值
    dst.g = src.g
    dst.r = src.r
...
class VConvolutionFilter(object):
    """
    A filter that applies a convolution to V.卷积滤波器
    """
...
class SharpenFilter(VConvolutionFilter):
    """
    A sharpen filter with a 1-pixel radius.锐化核
    """
...
class FindEdgesFilter(VConvolutionFilter):
    """
    An edge-finding filter with a 1-pixel radius.边缘检测核
    """
...
class BlurFilter(VConvolutionFilter):
    """
    A blur filter with a 2-pixel radius.模糊滤波器
    """
...
class EmbossFilter(VConvolutionFilter):
    """
    An emboss filter with a 1-pixel radius.浮雕滤镜
    """
